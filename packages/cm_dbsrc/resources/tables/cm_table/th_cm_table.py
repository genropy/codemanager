...
class View(BaseComponent):
    ...
    def th_queryBySample(self):
        return dict(fields=[dict(field='@pkg_id.code', lbl='Package', width='12em'),
    ...
    def th_top_sup(self,top):
        top.slotToolbar('5,sections@pkg_id,*,sections@groups,5',
                       childname='superiore',
    ...
    def sectionGroups(self):
        groups= self.db.table('cm_dbsrc.cm_table').query('$group',distinct=True, 
...
class Form(BaseComponent):
    ...
    def th_form(self, form):
        ...
        tc.contentPane(title='Outgoing Relations').plainTableHandler(table='cm_dbsrc.cm_relation',
                                                            view_store_dbenv_current_variant_code='=#FORM.record.cm_variant',
                                                            nodeId='outgoing_relations',
                                                            datapath='outgoing_relations',
                                                            condition='@relation_column.table_id=:tbl_id',
                                                            condition_tbl_id='^#FORM.pkey')

        tc.contentPane(title='Incoming Relations').plainTableHandler(table='cm_dbsrc.cm_relation',
                                                            nodeId='incoming_relations',
                                                            datapath='incoming_relations',
                                                            view_store_dbenv_current_variant_code='=#FORM.record.cm_variant',
                                                            condition='@related_column.table_id=:tbl_id',
                                                            condition_tbl_id='^#FORM.pkey')
