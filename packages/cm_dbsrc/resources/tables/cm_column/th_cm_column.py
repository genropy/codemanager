...
class View(BaseComponent):
    ...
    def th_queryBySample(self):
        return dict(fields=[dict(field='@table_id.sqlname', lbl='Table', width='12em'),
...
class ViewFromTable(View):
    ...
    def sectionTypes(self):
        types = self.db.table('cm_dbsrc.cm_column').query('$data_type', distinct=True, 
    ...
    def sectionGroups(self, table_id, **kwargs):
        if not table_id:
            return []
        groups= self.db.table('cm_dbsrc.cm_column').query('$group',
                                                      distinct=True, 
                                                where= '$group IS NOT NULL AND $table_id=:tbl_id',
...
class Form(BaseComponent):
    ...
    def th_form(self, form):
        ...
        fb.field('table_id', readOnly=True, background='rgba(128, 128, 128, 0.125)' )
        fb.field('name')
        fb.field('data_type')
        fb.field('old_type')
        fb.field('group')
