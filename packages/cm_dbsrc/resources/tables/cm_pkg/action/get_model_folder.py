...
class Main(BaseResourceAction):
    ...
    def pre_process(self):
        pkg_rec = self.get_selection().data[0]
        pkg = pkg_rec['code']
        pkg_id = pkg_rec['pkey']
        self.resultNode= self.page.site.storageNode('page:{}'.format(pkg))
        self.zipNode= self.page.site.storageNode('page:{}.zip'.format(pkg))
        self.tables_records = self.db.table('cm_dbsrc.cm_table').query(where='$pkg_id=:pkg_id',pkg_id=pkg_id).fetch()
        self.columns_records = self.db.table('cm_dbsrc.cm_column'
                                ).query(where='@table_id.pkg_id=:pkg_id',
                                        pkg_id=pkg_id,
                                        bagFields=True).fetchGrouped('table_id')
        self.relations_records = self.db.table('cm_dbsrc.cm_relation'
                                ).query(where='@relation_column.@table_id.pkg_id=:pkg_id',pkg_id=pkg_id).fetchGrouped('relation_column')
